...
public class ExamplesUniverse extends TestCase {
    ...
    Frog frogger = new Frog(new CartPt(180, 175), 7);
    Frog frogger2 = new Frog(new CartPt(225, 75), 9);
    Frog frogOffScreen = new Frog(new CartPt(650, 75), 9);
    
    Log logSlide = new Log(new CartPt(275, 475));
    ...
    Universe testinit;
    Universe testinit2;
    Universe testOffScreenu;
    Universe testWorldEnd;
    ...
    void initData() {
        ...
        testinit = new Universe(3, 0, frogger, start.lanes);
        testinit2 = new Universe(3, 0, frogger2, start.lanes);
        testOffScreenu = new Universe(1, 0, frogOffScreen, exlanes);
        testWorldEnd = new Universe(0, 0, frogOffScreen, exlanes);
    ...
    public void testImagesT(Tester t) {
        ...
        //world images
        t.checkExpect(start.lastImage(), start.lastImage());
        t.checkExpect(start.makeImage(), start.makeImage());
    ...
    //start using tests from the initial world
    public void testMoveOnLilog(Tester t)  {
        //moves when on log
        frogger2.lane = 9;
        t.checkExpect(frogger.loc.x, 180);
        testinit.moveOnLilog();
        t.checkExpect(frogger.loc.x, 178);
        
        //does not move when not on log
        t.checkExpect(frogger2.loc.x, 225);
        testinit2.moveOnLilog();
        t.checkExpect(frogger2.loc.x, 225);
        
    }
    
    //test off screen
    public void testOffScreen(Tester t) {
        CartPt cp1 = new CartPt(225, 225);
        CartPt cp2 = new CartPt(-100, 225);
        CartPt cp3 = new CartPt(225, -100);
        CartPt cp4 = new CartPt(650, 225);
        CartPt cp5 = new CartPt(225, 650);
        
        t.checkExpect(cp1.offScreen(), false);
        t.checkExpect(cp2.offScreen(), true);
        t.checkExpect(cp3.offScreen(), true);
        t.checkExpect(cp4.offScreen(), true);
        t.checkExpect(cp5.offScreen(), true);
    ...
    public void testWin(Tester t) {
        frogger2.lane = 9;
        testinit.win();
        t.checkExpect(testinit2.score, 0);
        t.checkExpect(frogger2.lane, 9);
        frogger2.lane = 10;
        testinit2.win();
        t.checkExpect(testinit2.score, 1);
        t.checkExpect(frogger2.lane, 0);
    }
    
    public void testOnKey(Tester t) {
        //test 
        t.checkExpect(testinit.lives, 3);
        t.checkExpect(testinit.frog.lane, 7);
        testinit.onKeyEvent("up"); 
        t.checkExpect(testinit.lives, 3);
        t.checkExpect(testinit.frog.lane, 8);

        t.checkExpect(testOffScreenu.lives, 1);
        t.checkExpect(testOffScreenu.frog.lane, 9);
        testOffScreenu.onKeyEvent("right");
        t.checkExpect(testOffScreenu.lives, 0);
        t.checkExpect(testOffScreenu.frog.lane, 0);
        
    }
    
    //test on Tick
    //all methods of ontick and move objects have been tested by themselves
    ...
    public void testWorldEnds(Tester t) {
        WorldEnd we1 = new WorldEnd (true, testWorldEnd.lastImage());
        WorldEnd we2 = new WorldEnd(false, testu.makeImage());
        
        t.checkExpect(testu.worldEnds(), we2);
        t.checkExpect(testWorldEnd.worldEnds(), we1);
